    ...
    <View style={s.container}>
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.headerCard}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.headerTitle}>AI Part Identifier</Text>
            <Text style={s.headerDesc}>Photograph or describe a part and AI will identify it</Text>
          </View>
        </View>
        ...
        {vehicles && vehicles.length > 0 && (
          <>
            <Text style={s.sectionLabel}>Vehicle Context <Text style={s.optional}>(optional)</Text></Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 4 }}>
              {vehicles.map((v) => (
                <Pressable
                  key={v.id}
                  style={[s.vehicleChip, selectedVehicleId === v.id && s.vehicleChipActive]}
                  onPress={() => setSelectedVehicleId(selectedVehicleId === v.id ? null : v.id)}
                >
                  <Text style={[s.vehicleChipText, selectedVehicleId === v.id && s.vehicleChipTextActive]}>
        ...
        {/* Image section */}
        <Text style={s.sectionLabel}>Photo <Text style={s.optional}>(optional, improves accuracy)</Text></Text>
        {pickedImage ? (
          <View style={s.imagePreviewWrap}>
            <Image source={{ uri: pickedImage.uri }} style={s.imagePreview} resizeMode="cover" />
            <Pressable style={s.imageRemoveBtn} onPress={() => setPickedImage(null)}>
              <MaterialCommunityIcons name="close-circle" size={26} color="#fff" />
            </Pressable>
            <Pressable style={s.imageChangeBtn} onPress={showImageOptions}>
              <MaterialCommunityIcons name="camera-retake" size={16} color="#fff" />
              <Text style={s.imageChangeBtnText}>Change</Text>
            </Pressable>
          </View>
        ) : (
          <View style={s.imagePickerRow}>
            <Pressable style={s.imagePickerBtn} onPress={handleTakePhoto}>
              <MaterialCommunityIcons name="camera" size={24} color="#7C3AED" />
              <Text style={s.imagePickerBtnText}>Take Photo</Text>
            </Pressable>
            <Pressable style={s.imagePickerBtn} onPress={handlePickImage}>
              <MaterialCommunityIcons name="image-multiple" size={24} color="#7C3AED" />
              <Text style={s.imagePickerBtnText}>From Library</Text>
            </Pressable>
          </View>
        )}

        {/* Description */}
        <Text style={s.sectionLabel}>Describe the Part <Text style={s.optional}>{pickedImage ? "(optional if photo added)" : ""}</Text></Text>
        <TextInput
          style={s.textArea}
          multiline
          numberOfLines={4}
        ...
        <Pressable
          style={[s.identifyBtn, (!canIdentify || loading) && { opacity: 0.6 }]}
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <Text style={s.identifyBtnText}>Identify Part</Text>
            </>
          )}
        </Pressable>
        ...
        {result && (
          <View style={s.resultCard}>
            <View style={s.resultHeader}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={s.partName}>{result.partName}</Text>
                {result.partNumber && result.partNumber !== "N/A" && (
                  <Text style={s.partNumber}>Part #: {result.partNumber}</Text>
                )}
              </View>
            </View>

            <Text style={s.resultLabel}>Description</Text>
            <Text style={s.resultText}>{result.partDescription}</Text>

            <Text style={s.resultLabel}>Function</Text>
            <Text style={s.resultText}>{result.function}</Text>

            <Text style={s.resultLabel}>Location</Text>
            <Text style={s.resultText}>{result.location}</Text>

            {result.commonIssues?.length > 0 && (
              <>
                <Text style={s.resultLabel}>Common Issues</Text>
            ...
            <View style={s.costRow}>
              <View style={s.costItem}>
                ...
                <Text style={s.costValue}>{result.estimatedCost}</Text>
              </View>
              <View style={s.costItem}>
                <Text style={s.costLabel}>Difficulty</Text>
                <Text style={s.costValue}>{result.replacementDifficulty}</Text>
              </View>
            </View>
          </View>
        )}
