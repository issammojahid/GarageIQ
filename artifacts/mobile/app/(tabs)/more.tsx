  ...
  return (
    <View style={[styles.container, { paddingTop: topPad }]}>
      <View style={styles.header}>
        <Text style={[styles.headerTitle, isRTL && styles.textRight]}>{t("more_title")}</Text>
      </View>
      <ScrollView
      ...
      >
        <View style={styles.grid}>
          {topRow.map((item) => (
            <Pressable
              key={item.id}
              style={({ pressed }) => [styles.gridItem, pressed && { opacity: 0.85, transform: [{ scale: 0.97 }] }]}
              onPress={() => router.push(item.route as Parameters<typeof router.push>[0])}
            >
              <View style={[styles.iconWrap, { backgroundColor: item.bgColor }]}>
                <MaterialCommunityIcons name={item.icon} size={28} color={item.color} />
              </View>
              <Text style={[styles.itemLabel, isRTL && styles.textRight]}>{t(item.labelKey)}</Text>
              <Text style={[styles.itemDesc, isRTL && styles.textRight]} numberOfLines={1}>{t(item.descKey)}</Text>
            </Pressable>
          ))}
        </View>
        ...
        {bottomRow.map((item) => (
          <Pressable
            key={item.id}
            style={({ pressed }) => [styles.listItem, isRTL && styles.rowReverse, pressed && { opacity: 0.85 }]}
            onPress={() => router.push(item.route as Parameters<typeof router.push>[0])}
          >
            ...
            <View style={styles.listItemContent}>
              <Text style={[styles.listItemLabel, isRTL && styles.textRight]}>{t(item.labelKey)}</Text>
              <Text style={[styles.listItemDesc, isRTL && styles.textRight]}>{t(item.descKey)}</Text>
            </View>
